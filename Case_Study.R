...
> compare_df_cols(jan,feb,mar,apr,may,jun,jul,aug,sep,oct,nov,sep,return = "mismatch")
...
> bike_rides_2022 <- bike_rides_2022 %>%
     mutate(
             ride_length_hours = as.numeric(difftime(ended_at, started_at, units = "hours")),
...
> bike_rides_2022 <- bike_rides_2022 %>%
      mutate(
              day_of_week_letter = wday(started_at, abbr = TRUE, label = TRUE),
              day_of_week_number = wday(started_at)
            )
